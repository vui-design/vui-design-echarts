export default {
  "header": {
    "placeholder": "搜索组件..."
  },
  "menu": {
    "install": "安装",
    "getting-started": "快速上手",
    "use-with-vue-cli": "在 vue-cli 中使用",
    "theme": "定制主题",
    "i18n": "国际化",
    "changelog": "更新日志",
    "echarts": "图表",
    "echarts-line": "折线图"
  },
  "anchors": {
    "title": "目录",
    "example-api": "API",

    "example-install-npm": "NPM 安装",
    "example-install-cdn": "CDN 引入",

    "example-getting-started-import": "引入 Vui Design",
    "example-getting-started-import-1": "1. 安装脚手架工具",
    "example-getting-started-import-2": "2. 创建一个项目",
    "example-getting-started-import-3": "3. 使用组件",
    "example-getting-started-import-4": "4. 组件列表",
    "example-getting-started-babel-plugin-import": "按需加载",
    "example-getting-started-tips": "小贴士",

    "example-use-with-vue-cli-install": "安装和初始化",
    "example-use-with-vue-cli-import-vui-design": "引入 Vui Design",
    "example-use-with-vue-cli-advanced-configuration": "高级配置",
    "example-use-with-vue-cli-babel-plugin-import": "1. 使用 babel-plugin-import",
    "example-use-with-vue-cli2": "1.1 使用 vue-cli 2 的小伙伴",
    "example-use-with-vue-cli3": "1.2 使用 vue-cli 3 的小伙伴",

    "example-line-basic-usage": "基础折线图",
    "example-line-double": "双折线",
    "example-line-smooth": "基础平滑折线图",
    "example-line-area": "基础面积图",
    "example-line-stack": "堆叠折线图",
    "example-line-area-stack": "堆叠面积图",
    "example-line-area-stack-gradient": "渐变堆叠面积图",
    "example-line-marker": "未来一周气温变化",
    "example-line-area-pieces": "折线图区域高亮",
    "example-line-gradient": "折线图的渐变",
    "example-line-sections": "一天用电量分布",
    "example-line-big-data": "大数据量面积图",
    "example-line-confidence-band": "Confidence Band",

    "example-bar-basic-usage": "基础柱状图",
    "example-bar-background": "带背景色的柱状图",
    "example-bar-tick-align": "坐标轴刻度与标签对齐",
    "example-bar-data-color": "自定义单个柱子颜色",
    "example-bar-data-colors": "自定义每个柱子颜色",
    "example-bar-waterfall": "瀑布图（柱状图模拟）",
    "example-bar-waterfall2": "阶梯瀑布图（柱状图模拟）",
    "example-bar-negative": "交错正负轴标签",
    "example-bar-negative2": "正负条形图",
    "example-bar-yaxis-category": "世界人口总量 - 条形图",
    "example-bar-gradient": "特性示例：渐变色 阴影 点击缩放",
    "example-bar-label-rotation": "柱状图标签旋转",
    "example-bar-stack": "堆叠柱状图",
    "example-bar-yaxis-category-stack": "堆叠条形图",
    "example-bar-rainfall-vs-evaporation": "某地区蒸发量和降水量",
    "example-bar-mix-line-bar": "折柱混合",
    "example-bar-mix-zoom-on-value": "多数值轴缩放",
    "example-bar-multiple-yaxis": "多 Y 轴示例",
    "example-bar-animation-delay": "柱状图动画延迟",
    "example-bar-large": "大数据量柱图",
    "example-bar-race": "动态排序柱状图",
    "example-bar-dynamic-data": "动态数据",
  }
};